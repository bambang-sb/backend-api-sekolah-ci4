...
$(document).ready(function () {
  ...
  var nameUri = 'pengguna';
  function resetForm(){
    $("#username, #id-user, #password").val('');
  }
  ...
  $("#btn-tambah").click(function(){
    $('#title-modal').html("Form Tambah Pengguna");
    ...
    .then((data) => {
      ...
      if(data.message == 'Validation Failed'){
        ...
        if(data.errors.username !== undefined){
          data.errors.username.forEach(el => {
            err += `${el} <br/>`;
          });
        }
        if(data.errors.password !== undefined){
          data.errors.password.forEach(el => {
            err += `${el} <br/>`;
          });
        }
      ...
      if(data.statusCode !==201){
        var inputFM = $('#username').val();
        myNotify('Opss !!',`Username ${inputFM} Sudah ada ! `,'warning');
        return;
      }
      $("#form-modal").modal("hide");
      //tambah data ke tabel
      var currentData = table.rows().data().toArray();
      currentData.unshift([null,$('#username').val() ,$('#role option:selected').text(),action(data.data.id)]); // null untuk kolom nomor, yang akan dirender otomatis
  ...
  function getAllData() {
    ...
    .then(doc => {
      ...
      data.forEach(item => {
        // var currentData = table.rows().data().toArray();
        row.push([null,item.username, item.role,action(item.id_user)]); // null untuk kolom nomor, yang akan dirender otomatis
      });
      table.clear();
      table.rows.add(row);
      table.draw();
  ...
  //     $('#id-user').val(data.data.id_mata_pelajaran);
  //     $('#username').val(data.data.nama);
  ...
  $("#data-tabel").on('click','.btn-edit',function(){
    var id = $(this).data("set");
    myNotify('Opps!','Fitur Edit belum tersedia','warning');
